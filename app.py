...
@app.route('/ponto/<int:ponto_id>/alterar', methods=['GET', 'POST'])
def alterar_ponto(ponto_id):
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        return redirect(url_for('ver_meus_pontos', email=ponto_para_alterar.autor.email))

    return render_template('alterar_ponto.html', ponto=ponto_para_alterar)

@app.route('/ponto/<int:ponto_id>/remover', methods=['POST'])
def remover_ponto(ponto_id):
    ponto_para_remover = Ponto.query.get_or_404(ponto_id)
    email_do_autor = ponto_para_remover.autor.email
